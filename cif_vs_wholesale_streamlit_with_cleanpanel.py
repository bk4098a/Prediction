...
DEFAULT_DIRS = [
    BASE_DIR,
    Path.cwd(),                         # 현재 실행 폴더
    Path(__file__).parent,              # 스크립트 위치
    Path(__file__).parent / "samples",  # 샘플 폴더
    Path.cwd() / "data",                # ./data
...
def vecm_irf_plot(df, vec_lagmax, irf_h, girf=False):
    ...
    if pick is None:
        ...
        # statsmodels 버전에 따라 resid 형상 다름 → 행렬로 스택
        resid_mat = np.column_stack([res.resid[i] for i in range(len(res.resid))])
    ...
    if df is None or df.empty:
        ...
        if st.button("ECM-ARIMAX 실행", key="ax_run"):
            ...
            if best is None:
                ...
                lo_arr = np.asarray(ci.iloc[:,0]) if hasattr(ci,"iloc") else np.asarray(ci)[:,0]
                hi_arr = np.asarray(ci.iloc[:,1]) if hasattr(ci,"iloc") else np.asarray(ci)[:,1]
